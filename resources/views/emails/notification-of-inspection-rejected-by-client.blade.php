...
<x-email.layout :subject="$subject">
    <x-email.block>
        <x-email.section>
            ...
            <x-email.line>
                The client {{ $noi->assignment->project->client->business_name }} has rejected the inspection for assignment <i>{{ $noi->assignment->reference_number }}</i>.
            </x-email.line>
            ...
            @if($noi->proposed_from && $noi->proposed_to)
                <x-email.article>
                    <strong>Proposed Alternative Date:</strong><br />
                    <i>{{ $noi->proposed_from->timezone($timezone)->format('F j, Y H:i') }} to {{ $noi->proposed_to->timezone($timezone)->format('F j, Y H:i') }} ({{ $timezone }})</i>
                </x-email.article>
            @endif

            @if($noi->rejection_reason)
                <x-email.article>
                    <strong>Rejection Reason:</strong><br />
                    <i>{{ $noi->rejection_reason }}<br /></i>
                </x-email.article>
            @endif
